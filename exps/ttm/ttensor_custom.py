...
def get_singular_values(data: torch.Tensor,
        batch: Optional[bool] = False):
    s_values = []
    i = 0
    while (data.dim()>1):
        unfolding = tn.right_unfolding(data, batch)#tn.unfolding(data, 0, batch)
        shapes = list(data.shape)
        data = data.reshape(tuple([data.shape[0]* data.shape[1]] + shapes[2:]))
        i+=1
        u, s, vh = torch.linalg.svd(unfolding)
        s_values.append(s)
    return s_values
...
class CustomTensor(object):
    ...
    def __init__(
            ...
            batch: Optional[bool] = False,
            is_from_weight: Optional[bool] = False,
