...
class TTCompressedLinear(CompressedLinear):
    ...
    def get_proper_rank(self, matrix: T.Tensor) -> Sequence[float]:
        rank = self.rank
        shape = self.shape
        ndim = len(shape[0])
        ...
        print ("ten shape before", tensor.shape)
        i = 0
        while (i < tensor.dim()-1 ):
            shapes = list(tensor.shape)
            first = np.prod(shapes[0:i+1])
            second = np.prod(shapes[i+1:])
            print ("first, second", first, second)
            print ("shapes before", shapes)
            unfolding = tensor.reshape(first, second)
            print ("shapes after", unfolding.shape)
            i+=1
            u, s, vh = T.linalg.svd(unfolding)
            svals.append(s)
    ...
    @classmethod
    def from_linear(self, linear: T.nn.Linear,
